...
plot!(twinx(), ytick=(0:100:400, reverse(0:100:420)), ylims=(-20,400), ylabel="z (px)")
...
p3 = plot(v', c=cs', xlims=(1,20), legend=false, title="", ylims=(0,1.5), yticks=(0:0.5:1, 0:0.5:1), ylabel="g(z,t)",  xticks=(0:15:15, round.(Int,0:15/15:15/15) ), xlabel="Time (s)" )
annotate!(p3,-5, 1.45, text("C1", :center, 23, :black))
annotate!(p3, 10, 1.45, text("raw data", :center, 15, :black))
p4 = plot(v2', c=cs', xlims=(1,20), legend=false, title="", ylims=(0,1.5), yticks=(0:0.5:1, 0:0.5:1), ylabel="g(z,t)",  xticks=(0:15:90, round.(Int,0:15/15:90/15) ), xlabel="Time (s)" )
annotate!(p4, -5, 1.45, text("C2", :center, 23, :black))
annotate!(p4, 11, 1.45, text("5 frames (0.3 s) ", :center, 15, :black))
...
annotate!((-1., ll, text(L" x 10^{-2}")))
plot!(xticks=xticks, xlabel="z (mm)") ## z (mPixel)
...
p1 = heatmap(speeds[2:end], 1:42, 100*mat, c=:redsblues,
            ...
            colorbar_title="\nν"*L"_{c} (s^{-1})", cbarfontsize=12, title=" ", margin=5Plots.mm, rightmargin=14Plots.mm, xrotation=-45)
annotate!((9.5, 48, text(L" x 10^{-2}")))
...
p2 = heatmap(speeds[2:end], 1:42, mat, c=:redsblues,
             xlabel="ω (s⁻¹)", ylabel="z (mm)", yticks=(0:10:40, reverse(0:4:16)), 
            colorbar_title="\nν"*L"_{H} (s^{-1})", cbarfontsize=12, title=" ", margin=5Plots.mm, rightmargin=14Plots.mm, xrotation=-45, clims=(0,4))
...
p0 = scatter(my_speeds[2:end],100 .* taus[1:end], label="",c=:black, ylabel=L"\nu_c^{max} (s^{-1})", ms=12, xticks=:none, ylims=(0,5), topmargin=10Plots.mm,)
annotate!(-0.65, 5.5, text("A", :center, 23, :black))
annotate!((-0.1, 5.5, text(L" x 10^{-2}")))
annotate!(-0.65, -0.8, text("B", :center, 23, :black))
scatter!([],[],shape=:circle, label="", c=:black, msc=:black, ms=12, fg_legend=:transparent, legend=:topleft,)
...
fit_hhgg = curve_fit(linear, hh_max, gg_max, [1.,1.])
p =plot([0,10],linear([0,10],fit_hhgg.param), label="", c=:black, ls=:dash)
scatter!(hh_max, gg_max, label="", xlabel="H", ylabel="G", ylims=(10,17), xlims=(10,17), c=cs, msc=:auto, ms=10)
plot!(ylabel=L"Z_{max} "*" from "*L"\nu_H"*"(mm)", xlabel=L"Z_{max} "*" from "*L"\nu_c"*"(mm)", ylims=(0,8), xlims=(0,8), c=:black, ls=:dash, margin=5Plots.mm, rightmargin=14Plots.mm, legend=:topleft, bottommargin=10Plots.mm, topmargin=10Plots.mm)
